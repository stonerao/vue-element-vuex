...
export default {
  ...
  teaching_info() {
    ...
    var url = '';
    if(this.state!=2){
        url = state == '2' ? api.Teacher_teaching_info : api.teaching_info
    }else{ 
        url = state == '2' ? api.teacher_teaching_info_my : api.student_teaching_info
    }
    this.$http(url, {
      params: {
        token: getToken(),
      ...
      this.state!=2?this.datas = res.data.datas:this.datas = res.data.data;
    })
  },
  teaching_collect(id) {
    this.$http({
  ...
  teaching_list(state, status) {
    if (!state && state != '1') {
      return
    }
    var url = ''; 
    url = (state == '2') ? api.teaching_interface_Teacher : api.student_teaching_list; //判断接口  
    function time(time) {
