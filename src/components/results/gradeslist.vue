    ...
    <div>
        ...
        <el-table ref="multipleTable" :data="t_data" tooltip-effect="dark" style="width: 100%" @selection-change="selectOption">
            ...
            <el-table-column label="id" width="80" show-overflow-tooltip>
                <template scope="scope">{{ scope.row.le_id }}</template>
            </el-table-column>
            ...
            <el-table-column width="180" label="操作" show-overflow-tooltip>
                <template scope="scope">
                    <el-button size="mini" @click="getQues(scope.row.le_id)">查看</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ajax() {
            this.t_data = [];
            store.Studentline_gradeslist.call(this)
        },
        selectOption() {
