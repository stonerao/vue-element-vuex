        ...
        <div class='WordSection1'>
            ...
            <div id="for" v-for="(item,index) in t_data" :key="index">
                ...
                <div style='mso-element:para-border-div;border:none;border-bottom:solid #EDEDED 1.5pt;padding:0cm 0cm 11.0pt 0cm;margin-left:0cm;margin-right:24.0pt'>
                    ...
                    <p class='MsoNormal' v-if="item.q_option" style='mso-margin-top-alt:auto;mso-margin-bottom-alt:auto;line-height:19.5pt;border:none;mso-border-bottom-alt:solid #EDEDED 1.5pt;padding:0cm;mso-padding-alt:0cm 0cm 11.0pt 0cm'>
                        <b>试题答案： </b>
                        <span lang=EN-US>
                            <span v-if="item.q_type_id!='4'">{{item.q_type_id!='3'?item.answer:item.answer=='1'?'正确':'错误'}}</span>
                            <div v-if="item.q_type_id=='4'" style="margin-left:80px;margin-top:-25px">
                                {{item.q_option}}
                                <p v-for="an in item.q_option">第 {{an.index+1}}题： {{an.title}}</p>
                            </div>
                            <b>
